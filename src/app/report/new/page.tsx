...
export default function ReportNewPage() {
  ...
  const [step, setStep] = useState(1); // 1: location, 2: details, 3: photos, 4: review
  ...
  if (submitSuccess) {
    return (
      <div style={{
        ...
        justifyContent: 'center', padding: '1rem', background: 'var(--bg-primary)',
      }}>
        <div className="animate-fade-in" style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>
            Terima kasih atas laporan Anda. Tim kami akan memverifikasi segera.
          </p>
        </div>
      </div>
    );
    ...
    <div style={{ minHeight: '100dvh', background: 'var(--bg-primary)' }}>
      ...
      {submitError && (
        <div className="animate-slide-down" style={{
          margin: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem',
            ...
            <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div style={{ textAlign: 'center', padding: '1rem 0' }}>
                ...
                <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
                  Kami akan menggunakan GPS Anda untuk menentukan lokasi
                </p>
              </div>
              ...
              {isLocating ? (
                ...
                <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', overflow: 'visible' }}>
                  ...
                  <div>
                    <label htmlFor="address" style={{ display: 'block', fontSize: '0.8125rem', fontWeight: 500, marginBottom: '0.375rem', color: 'var(--text-secondary)' }}>
                      Alamat (opsional)
                    </label>
                    <input id="address" type="text" className="input"
                      placeholder="Jl. Contoh No. 123, Kelurahan..." {...register('address')} />
                  </div>
                </div>
              ) : null}
          ...
          {step === 2 && (
            <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div>
                ...
                <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
                  Pilih yang paling mendekati kondisi banjir
                </p>
              </div>
                ...
                {SEVERITY_OPTIONS.map((opt) => (
                  <button key={opt.value} type="button"
                    ...
                    style={{
                      ...
                      borderColor: watchSeverity === opt.value ? opt.color : 'var(--border-primary)',
                      background: watchSeverity === opt.value ? `${opt.color}10` : 'var(--bg-card)',
              ...
              {errors.severity && (
                <p style={{ fontSize: '0.75rem', color: '#ef4444' }}>{errors.severity.message}</p>
              )}

              {/* Water Height Presets */}
              <div>
                <label style={{ display: 'block', fontSize: '0.8125rem', fontWeight: 500, marginBottom: '0.5rem', color: 'var(--text-secondary)' }}>
                  Ketinggian Air (opsional)
                </label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                  {WATER_HEIGHT_PRESETS.map((preset) => (
                    <button key={preset.value} type="button"
                      onClick={() => setValue('water_height_cm', preset.value)}
                      className="btn btn-secondary"
                      style={{
                        fontSize: '0.75rem', padding: '0.5rem 0.75rem',
                        borderColor: watch('water_height_cm') === preset.value ? 'var(--primary-500)' : 'var(--border-primary)',
                        background: watch('water_height_cm') === preset.value ? 'rgba(59,130,246,0.1)' : 'var(--bg-elevated)',
                      }}
                    >
              ...
              <label style={{
                ...
                <input type="checkbox" {...register('is_surge_receding')}
              ...
              {/* Description */}
              <div>
                <label htmlFor="description" style={{ display: 'block', fontSize: '0.8125rem', fontWeight: 500, marginBottom: '0.375rem', color: 'var(--text-secondary)' }}>
                  Deskripsi (opsional)
                </label>
                <textarea id="description" className="input" rows={3}
                  placeholder="Kondisi banjir, dampak, kerusakan..."
                  style={{ resize: 'vertical', minHeight: '80px' }}
                  {...register('description')} />
              </div>

              <button type="button" onClick={() => setStep(3)} className="btn btn-primary"
                style={{ width: '100%' }} disabled={!watchSeverity}>
                Lanjut ke Foto
              </button>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div style={{ textAlign: 'center', padding: '1rem 0' }}>
                ...
                <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
                  Tambahkan hingga 3 foto (opsional, meningkatkan kredibilitas)
                </p>
              </div>
                ...
                {photos.length < 3 && (
                  <label style={{
                    ...
                    <input type="file" accept="image/*" capture="environment"
                      onChange={handlePhotoAdd} multiple style={{ display: 'none' }} />
                  </label>
                )}
              </div>

              <div style={{ display: 'flex', gap: '0.75rem' }}>
                <button type="button" onClick={() => setStep(4)} className="btn btn-secondary" style={{ flex: 1 }}>
                  Lewati
                </button>
                <button type="button" onClick={() => setStep(4)} className="btn btn-primary" style={{ flex: 1 }}
              ...
              <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem' }}>
                  <Droplets size={16} color={
                    SEVERITY_OPTIONS.find(o => o.value === watchSeverity)?.color || 'var(--text-muted)'
                  } style={{ marginTop: '2px', flexShrink: 0 }} />
                  <div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Tingkat Keparahan</p>
                    <span className={`badge badge-severity-${watchSeverity}`}>
                      {watchSeverity ? SEVERITY_LABELS[watchSeverity] : '-'}
                    </span>
                  </div>
                </div>
